...
export default function Ventas({ setPagina }) {
  ...
  async function cargarVentasSesion(prods) {
    ...
    const prodMap = {};
    (prods || productos).forEach(p => { prodMap[p.id] = p; });
    setVentasHoy(ventas.map(v => ({ ...v, producto: prodMap[v.productoId] })));
  }
    ...
    async function calcularStock() {
      ...
      if (resultado) {
        ...
        if (!costoEditadoRef.current) {
          setForm(f => ({ ...f, costoUnitario: resultado.costoPromedio.toFixed(2) }));
        }
      }
    }
